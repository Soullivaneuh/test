...
var Admin = {
    ...
    "log" () {
        ...
        const msg = `[Sonata.Admin] ${Array.prototype.join.call(arguments, ", ")}`;
    ...
    "add_filters" (subject) {
        ...
        jQuery("a.sonata-toggle-filter", subject).on("click", (e) => {
            ...
            const filters_container = jQuery(`#${jQuery(e.currentTarget).attr("filter-container")}`);
            ...
            let targetSelector = jQuery(e.currentTarget).attr("filter-target"),
        ...
        jQuery(".sonata-filter-form", subject).on("submit", function () {
            ...
            jQuery(this).find("[sonata-filter=\"true\"]:hidden :input").
                val("");

        });

        /* Advanced filters */
        if (jQuery(".advanced-filter :input:visible", subject).filter(function () {
        ...
        jQuery("[data-toggle=\"advanced-filter\"]", subject).click(() => {
    ...
    "set_object_field_value" (subject) {
        ...
        jQuery("a.sonata-ba-edit-inline", subject).click(function (event) {
            ...
            const subject = jQuery(this);
            jQuery.ajax({
                "url": subject.attr("href"),
                "type": "POST",
                "success" (response) {

                    const elm = jQuery(subject).parent();
                    elm.children().remove();
                ...
                "error" (xhr, statusText, errorThrown) {
                    ...
                    jQuery(subject).parent().
                        effect("highlight", {"color": "#C43C35"}, 2000);
    ...
    "setup_collection_counter" (subject) {
        ...
        jQuery(subject).find("[data-prototype]").
            each(function () {

                const collection = jQuery(this);
                let counter = 0;
                collection.children().each(function () {

                    const matches = highestCounterRegexp.exec(jQuery("[id^=\"sonata-ba-field-container\"]", this).attr("id"));
    ...
    "setup_collection_buttons" (subject) {
        ...
        jQuery(subject).on("click", ".sonata-collection-add", function (event) {
            ...
            const container = jQuery(this).closest("[data-prototype]");
            const counter = ++Admin.lolCounters[container.attr("id")];
        ...
        jQuery(subject).on("click", ".sonata-collection-delete", function (event) {
            ...
            jQuery(this).closest(".sonata-collection-row").
                remove();
    ...
    "switch_inline_form_errors" (subject) {
        ...
        let row = subject.closest(".sonata-ba-field-inline-table"),
    ...
    "setup_sortable_select2" (subject, data) {
        ...
        subject.select2("container").find("ul.select2-choices").
            sortable({
                "containment": "parent",
                "start" () {
        ...
        subject.parents("form:first").submit((event) => {
    ...
    "handleScroll" (footer, navbar, wrapper) {
        ...
        jQuery(window).scroll(Admin.debounce(() => {
        ...
        jQuery(window).resize(Admin.debounce(() => {
    ...
    "setup_readmore_elements" (subject) {
        ...
        jQuery(subject).find(".sonata-readmore").
            each(function (i, ui) {
...
jQuery(document).on("sonata-admin-append-form-element", (e) => {
